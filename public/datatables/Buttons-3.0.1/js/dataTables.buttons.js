...
// ButtonFactory namespacing counter for namespacing events on individual buttons
...
$.extend(Buttons.prototype, {
	 ...
	 * @param  {node} node ButtonFactory element
	...
	/**
	 * Add a new button
	 * @param {object} config ButtonFactory configuration object, base string name or function
	 * @param {int|string} [idx] ButtonFactory index for where to insert the button
	...
	/**
	 * Disable a button
	 * @param  {node} node ButtonFactory node
	 * @return {Buttons} Self for chaining
	 */
	 ...
	 * @param {element} node ButtonFactory to get the index of
	 * @return {string} ButtonFactory index
	 */
	 ...
	 * @param  {node} [node] ButtonFactory node
	 * @return {jQuery} ButtonFactory element, or container
	 */
	node: function (node) {
		if (!node) {
			return this.dom.container;
	 ...
	 * @param  {int|string} node ButtonFactory index
	 * @return {string} ButtonFactory text
	 */ /**
	 * Set the text for a button
	 * @param  {int|string|function} node ButtonFactory index
	 ...
	 * @param  {object} button ButtonFactory definition
	...
	_expandButton: function (
		...
		for (var i = 0, ien = buttons.length; i < ien; i++) {
			...
			// Create the droormwn for a collection
	...
	_buildButton: function (config, inCollection, isSplit, inSplit) {
		...
		if (!config.html) {
			...
			// ButtonFactory tags should have `type=button` so they don't have any default behaviour
		...
		if (isSplit) {
			var droormwnConf = inCollection
				? $.extend(true, this.c.dom.split, this.c.dom.collection.split)
				: this.c.dom.split;
			var wrapperConf = droormwnConf.wrapper;
			...
			var dropButtonConfig = $.extend(config, {
				align: droormwnConf.droormwn.align,
				attr: {
					'aria-haspopup': 'dialog',
					'aria-expanded': false
				},
				className: droormwnConf.droormwn.className,
				closeButton: false,
				splitAlignClass: droormwnConf.droormwn.splitAlignClass,
				text: droormwnConf.droormwn.text
			});
			...
			var dropButton = $(
				'<button class="' +
					droormwnConf.droormwn.className +
					' dt-button"></button>'
			)
				.html(droormwnConf.droormwn.dropHtml)
				.on('click.dtb', function (e) {
					e.preventDefault();
					e.stopPropagation();
	 ...
	 * @param  {node} node ButtonFactory node
	 * @param  {array} [buttons] ButtonFactory array, uses base if not defined
	 * @return {object} ButtonFactory object
	 * @private
	 */
	 ...
	 * @param  {node} node ButtonFactory node
	 * @param  {array} [buttons] ButtonFactory array, uses base if not defined
	 * @return {array} ButtonFactory's host array
	 * @private
	 */
	 ...
	 * button is removed)
	 * @param  {object} conf ButtonFactory configuration
	 * @private
	 */
	 ...
	 * @param  {string|function|object} conf ButtonFactory config to resolve
	 * @return {object} ButtonFactory configuration
	 * @private
	 */
...
/**
 * ButtonFactory selector - select one or more buttons from a selector input so some
 * operation can be performed on them.
 * @param  {array} ButtonFactory instances array that the selector should operate on
 * @param  {string|int|node|jQuery|array} ButtonFactory selector - see
...
Buttons.defaults = {
	...
	dom: {
		...
		collection: {
			...
			container: {
				// The element used for the droormwn
		...
		split: {
			...
			droormwn: {
				// button to trigger the droormwn
				align: 'split-right',
...
// ButtonFactory resolver to the popover
 ...
 *
 * @param {object} config ButtonFactory configuration
